...
char ps_read_char(FILE *port, psbool *eofp, long *status, psbool peekp) {
  psbool errorp;
  int result;
...
long ps_read_integer(FILE *port, psbool *eofp, long *status) {
  long result;
  int ch;
  psbool negate;
  psbool errorp;
...
long ps_write_char(char ch, FILE *port) {
...
long ps_write_integer(long n, FILE *port) {
  int status;
...
long ps_write_string(char *string, FILE *port) {
  while (1) {
...
long ps_read_block(FILE *port, char *buffer, long count, psbool *eofp,
                   long *status) {
  int got = 0;
  psbool errorp;
...
long ps_write_block(FILE *port, char *buffer, long count) {
  int sent = 0;
...
void ps_error(char *message, long count, ...) {
  va_list ap;
...
static FILE *ps_really_open_file(char *filename, long *status, char *mode) {
...
FILE *ps_open_input_file(char *name, long *status) {
  return ps_really_open_file(name, status, "r");
}

FILE *ps_open_output_file(char *name, long *status) {
  return ps_really_open_file(name, status, "w");
}

long ps_close(FILE *stream) {
  int status;
